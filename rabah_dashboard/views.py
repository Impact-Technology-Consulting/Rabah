...
class RabahHomePageView(View):
    def get(self, request):
        ...
        subscription_monthly = Subscription.objects.filter(
            subscription_duration="MONTHLY"
        ).first()
        subscription_quarterly = Subscription.objects.filter(
            subscription_duration="QUARTERLY"
        ).first()
        subscription_yearly = Subscription.objects.filter(
...
class UserOrganisationsView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        if member.count() == 0:
            ...
            logout(request)
            messages.error(
                request, "account have organisation attached to it please signup "
            )
...
class DashBoardView(AuthAndOrganizationMixin, View):
    ...
    def get(self, request):
        ...
        event_increment_percentage = Event.objects.calculate_event_increment_percentage(
            self.organisation_id
        )
        contribution_increment_percentage = (
            Contribution.objects.calculate_contribution_increment_percentage(
                self.organisation_id
            )
        )
        member_increment_percentage = (
            Member.objects.calculate_member_increment_percentage(self.organisation_id)
        )
        group_increment_percentage = Group.objects.calculate_group_increment_percentage(
            self.organisation_id
        )
        context = {
            ...
            "contribution_increment_percentage": round(
                contribution_increment_percentage, 2
            ),
