...
def check_and_cancel_inactive_subscription():
    for organisation_subscription in OrganisationSubscription.objects.filter(status="ACTIVE"):
        # check on stripe
        if not organisation_subscription.stripe_subscription_id:
            continue

        stripe_subscription_response = stripe.Subscription.retrieve(organisation_subscription.stripe_subscription_id)
