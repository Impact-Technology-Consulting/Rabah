...
class MemberUploadCreateForm(forms.Form):
    ...
    groups = forms.ModelMultipleChoiceField(
        queryset=Group.objects.all(), required=False, widget=CheckboxSelectMultiple
    )
...
class MemberEditForm(forms.ModelForm):
    class Meta:
        model = Member
        fields = ["family", "family_relationship", "is_owner", "groups"]
        widgets = {
...
class AddExistingMemberToFamilyForm(forms.Form):
    ...
    def __init__(self, organisation_id, current_member_id, *args, **kwargs):
        ...
        self.fields["current_member_id"] = forms.CharField(
            widget=forms.HiddenInput(), initial=current_member_id
        )
        self.fields["organisation_id"] = forms.CharField(
            widget=forms.HiddenInput(), initial=organisation_id
        )
    ...
    def save(self):
        current_member = Member.objects.filter(
            id=self.cleaned_data["current_member_id"],
        ...
        new_member = Member.objects.filter(
            id=self.cleaned_data["new_member_id"],
            organisation_id=self.cleaned_data["organisation_id"],
        ).first()
        if not new_member:
            return None
...
class UpdateExistingMemberFamilyRelationShipForm(forms.Form):
    ...
    def save(self):
        member = Member.objects.filter(
            id=self.cleaned_data["family_relationship_member_id"]
        ).first()
        if not member:
            return None
